...
- [ ] 3.1.1 ~ 3.1.8 逐包改造 (每次一个包，242处总计):
  - Controller 层 (7个)
  - Chat Service 层
  - User Service 层
  - Common Service 层
  - AI Service 层
  - Consumer/RocketMQ 监听器层
  - Event Listener 层
  - Config 层 → 保持 @Bean 方法不变
- [ ] 3.1.9 验证: `./mvnw clean compile`

### 3.2 java.util.Date → java.time
- [ ] 3.2.1 `DateUtils.java`: Calendar → java.time.temporal.ChronoUnit
- [ ] 3.2.2 `User.java`: Date → LocalDateTime
- [ ] 3.2.3 `Message.java`: Date → LocalDateTime
- [ ] 3.2.4 其余 Entity (Contact, GroupMember, Room 等): Date → LocalDateTime
- [ ] 3.2.5 DAO 层适配 (Date → LocalDateTime)
- [ ] 3.2.6 Service 层适配
- [ ] 3.2.7 `JwtUtils.java`: 适配 jjwt 0.12 + LocalDateTime
- [ ] 3.2.8 `MinIOTemplate.java`: Date → LocalDateTime
- [ ] 3.2.9 `application.yml`: `write-dates-as-timestamps: false`
- [ ] 3.2.10 验证: `./mvnw clean compile`

### 3.3 清理
- [ ] 3.3.1 移除 `ChatMemberStatisticResp.java` `@Deprecated` totalNum 字段
- [ ] 3.3.2 处理 `ChatGLM2Handler.java` 空 TODO
- [ ] 3.3.3 处理 `GroupMemberServiceImpl.java` 群聊删除通知 TODO
- [ ] 3.3.4 `logback.xml`: 添加 AsyncAppender
- [ ] 3.3.5 Git 提交: `refactor(phase3): constructor injection + java.time + cleanup`
