...
- [ ] 3.2.1 添加 `jackson-datatype-jsr310` 依赖 + Jackson 配置 (LocalDateTime → 毫秒时间戳，保持 API 兼容)
- [ ] 3.2.2 18 个 Entity: `Date` → `LocalDateTime` (MyBatis-Plus 3.5.9 内置 LocalDateTimeTypeHandler, MySQL datetime(3) 原生支持)
- [ ] 3.2.3 DAO 层 (3): MessageDao, ContactDao, RoomDao — Date 参数 → LocalDateTime
- [ ] 3.2.4 VO/DTO 层 (6): ChatRoomResp, ChatMessageResp, WSMemberChange, ChatMemberResp, RoomBaseInfo, MsgRecall
- [ ] 3.2.5 `DateUtils.getEndTimeByToday()` → `LocalDateTime` + `ChronoUnit` (仅 1 个调用方: GPTChatAIHandler)
- [ ] 3.2.6 Service/Util 层 (8): IpServiceImpl, WebSocketServiceImpl, RecallMsgHandler, ChatGLM2Handler, HotRoomCache, CursorUtils, ContactMapper — 适配
- [ ] 3.2.7 **跳过** — JwtUtils 使用 auth0 java-jwt 非 jjwt，`new Date()` 保持不动
- [ ] 3.2.8 **跳过** — MinIOTemplate 未见 Date 使用
- [ ] 3.2.9 **跳过** — `write-dates-as-timestamps: true` 保持不变 (3.2.1 通过 jsr310 模块确保 LocalDateTime 序列化为毫秒戳)
