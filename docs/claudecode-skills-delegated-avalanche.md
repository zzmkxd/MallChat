...
- [ ] 4.2.1 MySQL 8.0 容器: utf8mb4 charset/collation + `docs/mallchat.sql` 自动初始化
- [ ] 4.2.2 Redis 7 容器
- [ ] 4.2.3 MinIO 容器
- [ ] 4.2.4 RocketMQ NameServer + Broker 容器 (官方镜像 `apache/rocketmq`)
- [ ] 4.2.5 App 容器: mallchat-chat-server, 端口 8080+8090

### 4.3 配置注入 (混合方案)
- [ ] 4.3.1 创建 `application-docker.yml`: Spring Docker 环境配置 (log, datasource pool, actuator)
- [ ] 4.3.2 Docker Compose `environment:` 注入敏感值 (mysql password, jwt.secret, wx key)
- [ ] 4.3.3 验证: `docker-compose up -d` → `curl localhost:8080/actuator/health`
