...
- [x] 1.8.3 验证: `./mvnw clean compile` → 修复 4 类错误后全模块通过

### 1.9 Spring Boot 3 配置迁移
- [x] 1.9.1 `application.yml`: 删除 `spring.mvc.pathmatch.matching-strategy`
- [x] 1.9.2 `application.yml`: 检查 `spring.redis` 配置兼容性
- [x] 1.9.3 `logback.xml`: 修正日期格式 `dd-MM-yyyy` → `yyyy-MM-dd`
- [x] 1.9.4 验证: `./mvnw clean package -DskipTests` ✅ (8/8)

### 1.10 收尾
- [x] 1.10.1 `CLAUDE.md`: 更新版本号
- [x] 1.10.2 Git 提交: `93d72e8` — Phase 1 完成
...
- [x] 2.1.1 引入依赖: `springdoc-openapi-starter-webmvc-ui` + `knife4j-openapi3-jakarta-spring-boot-starter`
- [x] 2.1.2 重写 `SwaggerConfig.java` → `OpenApiConfig.java`
- [x] 2.1.3 ~ 2.1.8 逐包替换注解 (77 文件): `@Api`→`@Tag`, `@ApiOperation`→`@Operation`, `@ApiModelProperty`→`@Schema`
- [~] 2.1.0 ⚠️ **Phase 4 修正**: springdoc 2.7.0→`2.6.0`, knife4j 4.5.0→`4.4.0` (2.7.0 需 SB 3.4+, Phase 4 运行时才暴露)
  - Controller 层 (7个)
...
### 2.2 JWT 库清理 ⚠️ 计划偏差
- [~] 2.2.1 `JwtUtils.java`: **跳过** — 项目实际用 auth0 java-jwt (非 jjwt), 天然兼容 SB3
- [x] 2.2.2 移除 jjwt 依赖 (零引用): `pom.xml` ×2
- [~] 2.2.3 `LoginServiceImpl.java`: **跳过** — 无需适配
- [x] 2.2.4 验证: `./mvnw clean compile` ✅

### 2.3 JUnit 4 → JUnit 5
- [x] 2.3.1 `chat-server/pom.xml`: junit:junit + spring-test → spring-boot-starter-test
- [x] 2.3.2 `DaoTest.java`: `@RunWith`→`@ExtendWith`, `@Test` 包路径
- [x] 2.3.3 `SensitiveTest.java`: 同上
- [~] 2.3.4 `ScanHandler.java` + `SubscribeHandler.java`: **跳过** — src/main 中无 @Test (计划误判)
- [x] 2.3.5 验证: `./mvnw test-compile` ✅

### 2.4 收尾
- [x] 2.4.1 全局清理未使用的旧 import (无残留)
- [x] 2.4.2 Git 提交: `080d137` — Phase 2 完成
...
- [x] 3.2.8 **补漏完成** — MinIOTemplate Date→LocalDateTime (1处)
- [x] 3.2.9 `write-dates-as-timestamps: true` 保持
- [x] 3.2.10 验证: `./mvnw clean compile + test-compile` ✅
- [x] 3.2.11 **补漏完成** (2026-05-26, commit `7f67faa`) — mallchat-transaction (4文件) + oss-starter (1文件) Date→LocalDateTime
...
### 4.0 全链路编码统一
- [x] 4.0.1 `logback.xml`: 3 个 encoder 添加 `<charset>UTF-8</charset>`
- [x] 4.0.2 `application.yml` JDBC URL: 追加 `&connectionCollation=utf8mb4_unicode_ci`
- [x] 4.0.3 `docs/mallchat.sql` `wx_msg` 表: `utf8mb4_general_ci` → `utf8mb4_unicode_ci`
- [x] 4.0.4 验证: `./mvnw clean compile` ✅ — 待提交

### 4.1 Dockerfile
- [x] 4.1.1 创建单阶段 `Dockerfile` (eclipse-temurin:21-jre + COPY 预构建 JAR)
- [x] 4.1.2 Dockerfile 内置 UTF-8 环境: `ENV LANG=C.UTF-8` + `-Dfile.encoding=UTF-8`
- [x] 4.1.3 创建 `.dockerignore`
- [x] 4.1.4 验证: `docker build` ✅ — 多阶段→单阶段 (Maven in Docker OOM, 本机构建 JAR)

### 4.2 docker-compose.yml
- [x] 4.2.1 MySQL 8.0 容器: utf8mb4 (端口偏移 3307), mallchat.sql 自动初始化 ✅
- [x] 4.2.2 Redis 7 容器 (端口偏移 6380) ✅
- [x] 4.2.3 MinIO 容器 (端口偏移 9002/9003) ✅
- [x] 4.2.4 RocketMQ NameServer + Broker (端口偏移 9877, 健康检查移除) ✅
- [x] 4.2.5 App 容器: ✅ 启动成功, `curl localhost:8080/actuator/health` → `{"status":"UP"}`
- [x] 4.2b MyBatis-Plus 修复: `mybatis-plus-boot-starter`→`mybatis-plus-spring-boot3-starter:3.5.10.1`
- [x] 4.2c RedissonConfig 修复: `RedisProperties`→`@Value` 直接注入 (env var 传递问题)

### 4.3 配置注入 (混合方案)
- [x] 4.3.1 创建 `application-docker.yml` ✅
- [x] 4.3.2 Docker Compose `environment:`: `SPRING_REDIS_*` + `SPRING_DATASOURCE_URL` + `SPRING_PROFILES_ACTIVE` ✅
- [x] 4.3.3 验证: `docker compose up -d` → `curl /actuator/health` → `{"status":"UP"}` ✅

### 4.2b 阻塞修复: MyBatis-Plus + RedissonConfig ✅
- [x] 4.2b.1 MP 修复: `mybatis-plus-boot-starter` → `mybatis-plus-spring-boot3-starter:3.5.10.1`
- [x] 4.2b.2 Phase 1 LambdaUtils / count()→Long / jsqlparser 三处未受影响, 编译通过
- [x] 4.2b.3 RedissonConfig: `RedisProperties`→`@Value` (env var 不传 RedisProperties 的 bug)
- [x] 4.2b.4 springdoc: 2.7.0→2.6.0 + knife4j 4.5→4.4 (SB 3.3.5 兼容)
- [x] 4.2b.5 验证: `docker compose up` → `{"status":"UP"}`

### 4.4 CI/CD 骨架
- [x] 4.4.1 创建 `.github/workflows/build.yml` (编译 + test-compile, JDK 21, ubuntu-latest)
- [ ] 4.4.2 验证: Push 触发 (需实际推送到 GitHub)

### 4.5 收尾
- [ ] 4.5.1 更新 `CLAUDE.md` 添加 Docker 命令
- [ ] 4.5.2 Git 提交: `feat(phase4): Docker + docker-compose + CI/CD + 编码统一`
...
```bash
# Phase 1-3: 编译
./mvnw clean compile -pl mallchat-chat-server -am
